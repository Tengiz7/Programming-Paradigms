...
(define (intersection-points circles)
  (if (or (null? circles) (= (length circles) 1)) '() 
    (append 
      (apply append (map (lambda (x) (intersect (car circles) x)) (cdr circles)))
...
(define (distance-product point points)
  (apply * (map (lambda (n-point) (dist point n-point))
    (remove point points))
  )
)

(define (rate-points points)
  (map (lambda (point) (list (distance-product point points) point))
    points)
)
...
(define (clumped-points points)
  (apply append (map cdr (prefix-of-list (quicksort (rate-points points))
...
(define (average-point points)
    (let (
            (avg-point
                (make-pt (/ (apply + (map x points)) (length points)) (/ (apply + (map y points)) (length points)))
            )
        )
        (list (distance-product avg-point points) avg-point)
    )
)
