          ...
          int numWorkers)
{
  ...
  bank->nHasToFinish = numWorkers;
  sem_init(&(bank->checking),0,1);
  sem_init(&(bank->nextDay),0,0);
  sem_init(&(bank->lockForTrans),0,1);
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(int i = 0; i < bank->numberBranches; i++){
    sem_wait(&(bank->branches[i].brLock));
  }
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for(int i = 0; i < bank->numberBranches; i++){
        sem_post(&(bank->branches[i].brLock));
      }
      return err;
    }
