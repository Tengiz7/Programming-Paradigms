...
void generateEachSentence(string str,map<string, Definition>& grammar){
  if(str[0] != '<'){
    cout << str << " ";
    return;
  }

  Production prod = grammar[str].getRandomProduction();
  for (Production::iterator curr = prod.begin(); curr != prod.end(); curr++){
    generateEachSentence(*curr,grammar);
  }
}

void generateSentences(map<string, Definition>& grammar){
  for(int i = 0; i < 3; i++){
    cout << "Version #" << i + 1 << ": --------------" << endl;
    string starter = "<start>";
    generateEachSentence(starter, grammar);
    cout << endl;
  }
}
