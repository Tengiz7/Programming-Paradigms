...
def findOptimalAlignment(strand1, strand2, dic):
    if (strand1, strand2) in dic:
        return dic[(strand1, strand2)]
    # if one of the two strands is empty, then there is only
    # one possible alignment, and of course it's optimal
    if len(strand1) == 0: 
        dic[(strand1, strand2)] = (len(strand2) * -2, " " * len(strand2), strand2)
        return [len(strand2) * -2, " " * len(strand2), strand2]
    if len(strand2) == 0: 
        dic[(strand1, strand2)] = (len(strand1) * -2, " " * len(strand1) ,strand1)
    ...
    # they actually match.
    bestWith, str1, str2 = findOptimalAlignment(strand1[1:], strand2[1:], dic)
    if strand1[0] == strand2[0]:
        dic[(strand1, strand2)] = (bestWith + 1, strand1[0] + str1, strand2[0] + str2)
        return [bestWith + 1, strand1[0] + str1, strand2[0] + str2] # no benefit from making other recursive calls

    best = bestWith - 1
    str1, str2 = strand1[0] + str1, strand2[0] + str2
    # It's possible that the leading base of strand1 best
    # matches not the leading base of strand2, but the one after it.
    bestWithout, cur1, cur2 = findOptimalAlignment(strand1, strand2[1:], dic)
    ...
    # opposite scenario
    bestWithout, cur1, cur2  = findOptimalAlignment(strand1[1:], strand2, dic)
    bestWithout -= 2 # penalize for insertion of space
    if bestWithout > best:
        best = bestWithout
        str2 = " " + cur2
    ...
    dic[(strand1, strand2)] = (best, str1, str2)
...
def main():
    ...
    while (True):
        ...
        dic = {}
        alignment, str1, str2 = findOptimalAlignment(strand1, strand2, dic)
        rame1 = ""
        ...
        out.write(str2 + "\n")
        out.write(rame2 + "\n")
        test(alignment, rame1 , rame2, str1, str2)
